...
public class VisualiserTest {
	...
	public void testSom() throws Exception {
		Dataset dataset = manager.createDataBean("SOM", this.getClass().getResourceAsStream(TestConstants.SOM_CLUSTERED_RESOURCE));
	...
	public void testHC() throws Exception {
		...
		for (int i = 0; i < trees.length; i++) {
			Dataset tree = manager.createDataBean("HC clusters", this.getClass().getResourceAsStream(trees[i]));
			Dataset heatmap = manager.createDataBean("heatmap", this.getClass().getResourceAsStream(heatmaps[i]));
	...
	public void testVisualisations() throws Exception {
		...
		for (String resource : resources) {
			try {
				Dataset bean = manager.createDataBean(resource, this.getClass().getResourceAsStream(resource));
	...
	public void testScatterplot() throws Exception {
		for (String resource : new String[] {TestConstants.FOUR_CHIPS_RESOURCE, TestConstants.SCATTER_HARDCASE1, TestConstants.SCATTER_HARDCASE2}) {
			try {
				Dataset dataBean = manager.createDataBean(resource, this.getClass().getResourceAsStream(resource));
	...
	public void testHistogram() throws Exception {
		...
		Dataset dataBean = manager.createDataBean("Hist. data", this.getClass().getResourceAsStream(TestConstants.FOUR_CHIPS_RESOURCE));		
	...
	public void testExpressionProfile() throws Exception {
		...
		Dataset dataBean = manager.createDataBean("Profiledata", this.getClass().getResourceAsStream(TestConstants.FOUR_CHIPS_RESOURCE));
	...
	public void testClusteredProfiles() throws Exception {
		...
		Dataset dataBean = manager.createDataBean("Profiledata", this.getClass().getResourceAsStream(TestConstants.CLUSTERED_PROFILES_RESOURCE));
	...
	public void testApplicabilityChecks() throws MicroarrayException, IOException {
		Dataset affyMicroarray = manager.createDataBean("", this.getClass().getResourceAsStream(TestConstants.AFFY_RESOURCE));
	...
	private JComponent doVisualisation(VisualisationMethod method, Dataset bean) throws Exception {
