...
public class IntervalToolTest {
	...
	public void test() throws FileNotFoundException, IOException {
		...
		Assert.assertNotSame(tool.merge(rows1, rows2, 1L), expectedIntersection);
		...
		Assert.assertNotSame(tool.merge(rows1, rows2, 1L), expectedSubtraction);
		...
		Assert.assertEquals(tool.merge(rows1, rows2, 1L), expectedMerge);
		expectedMerge.remove();
		Assert.assertNotSame(tool.merge(rows1, rows2, 1L), expectedMerge);
		...
		Assert.assertNotSame(tool.merge(rows1, rows2, 51L), expectedSubtractionWithLongOverlap);
	}
