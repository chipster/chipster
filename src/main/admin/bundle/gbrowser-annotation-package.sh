...
# The second parameter defines the fasta file, which is moved to folder 'real-fasta' and replaced with a symbolic link.
# All annotation files of each species are then compressed to a tar package, which is placed under folder 'annotations'.
#
# When only two parameters are given, the first one defines the name of the packages and the search criteria of the 
# files that belong there. When more parameters are given, the first parameter defines only the package name and the 
# file to include are defined with third and fourth parameter.
#
...
#
# and finally run gbrowser-annotation-check.sh (it should complain only about replaced fasta files)
...
package "Rattus_norvegicus.RGSC3.4.69" 		"nochr/rn4.fa" 						"Rattus_norvegicus.RGSC3.4.69"	"rn4.fa*"
package "Rattus_norvegicus.Rnor_5.0.70"		"nochr/Rattus_norvegicus.Rnor_5.0.70.dna.toplevel.fa" 
package "R1-RT" 				"R1-RT.fa"
package "Sus_scrofa.Sscrofa10.2.70" 		"nochr/Sus_scrofa.Sscrofa10.2.69.dna.toplevel.fa"	"Sus_scrofa.Sscrofa10.2.70"	"Sus_scrofa.Sscrofa10.2.69.dna.toplevel.fa"
