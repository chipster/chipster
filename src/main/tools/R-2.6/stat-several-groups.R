...
if(meth=="empiricalBayes") {
   ...
   dat<-dat[rows,]
   write.table(na.omit(data.frame(dat, p.adjusted=round(p, digits=4))), file="multiple-sample.tsv", sep="\t", row.names=T, col.names=T, quote=F)
}
...
if(meth=="ANOVA") {
   ...
   write.table(na.omit(data.frame(dat, p.adjusted=round(p.adjusted, digits=4))), file="multiple-sample.tsv", sep="\t", row.names=T, col.names=T, quote=F)
}
