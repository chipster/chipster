...
# OUTPUT ngs-phenodata.tsv: "Experiment description file"
...
# Create large file
data_table <- array (dim = c(length(identifiers),number_files))
...
for (count in 1:number_files) {
	...
	data_table[indices, (count+annotation_columns)] <- get (paste ("data_", count, sep=""))[indices,3]
...
write.table(data.frame(sample=samples, experiment=experiment_type, group=groups), file="ngs-phenodata.tsv", sep="\t", row.names=F, col.names=T, quote=F)
