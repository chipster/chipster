# ANALYSIS Pathways/"miRNA, enriched GO" (Performs a statistical test for enrichmens of
...
if (species=="human") {
	targets <- read.table("mirna_mappings_hsapiens.txt", sep="\t")
}
if (species=="mouse") {
	targets <- read.table("mirna_mappings_mmusculus.txt", sep="\t")
}
if (species=="rat") {
	targets <- read.table("mirna_mappings_rnorvegious.txt", sep="\t")
}
...
if (summary.feature=="gene") {
	tran2gomf  <- read.table(file="go_molecular_function_gene.txt", sep="\t")
	tran2gobp  <- read.table(file="go_biological_process_gene.txt", sep="\t")
	tran2gocc  <- read.table(file="go_cellular_component_gene.txt", sep="\t")
}
if (summary.feature=="transcript") {
	tran2gomf  <- read.table(file="go_molecular_function_transcript.txt", sep="\t")
	tran2gobp  <- read.table(file="go_biological_process_transcript.txt", sep="\t")
	tran2gocc  <- read.table(file="go_cellular_component_transcript.txt", sep="\t")
}
