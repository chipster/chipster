...
# PARAMETER username STRING (Username, in case the data is password-protected. WARNING: This will store your username/password in the Chipster history files. To avoid this, use the session parameter.)
# PARAMETER password STRING (Password, in case the data is password-protected. WARNING: This will store your username/password in the Chipster history files. To avoid this, use the session parameter.)
# PARAMETER session STRING (Session ID. To avoid saving your username/password in Chipster history files, log in at http://www.cangem.org/ using a web browser, then copy&paste your session ID from the lower right corner of the CanGEM website. This will allow Chipster to access your password-protected data until you log out of the web site (or the session times out).)
...
if (cangem.samples$Format[1] == 'agilent') {
  ...
  if (is.na(chiptype)) {
    if (cangem.samples$Type[1] == 'miRNA Expression' || cangem.samples$Type[1] == 'miRNA') {
      chiptype <- 'miRNA'
    } else {
      chiptype <- 'cDNA'
    }
  ...
  names(dat)<-paste("chip.", microarrays, sep="") # or names(dat) ???
...
dat2 <- cbind(dat2, as.data.frame(dat)[dat2$probe,])
dat2$probe <- NULL
