...
public class MicroarrayGraph extends JGraph implements DataChangeListener, PropertyChangeListener {
	...
	public MicroarrayGraph(GraphModel model, GraphLayoutCache cache, GraphPanel graphPanel) {
		...
		/* Regardles of the updateUI-call jus before the font size of graph cell
		 * is fixed only when the first cell is added. To handle situations when the
		 * font size is changed before importing any data we force the jgraph to 
		 * initialise itself by inserting dummy vertex and removing it. This *should*
		 * happen before any repaint operation, and thus the vertex won't ever show. */ 
	 ...
	 * 
	 * @param data New DataBean to be inserted to this view.
	 */
	 ...
	 * 
	 * @param data Dataset to be removed.
	 * @return True if remove succeeded, false if it failed (that is,
	 * 		   corresponding vertex was not found in the graph).
	 */
	...
	/**
	 * Updates graph's selected vertexes. This is done by synchronizing selected 
	 * datas and graph cells. Update should be done when selections are 
	 * changes (of course) and also if groups are collapsed or expanded.
	 *
	 */
	...
	/**
	 * Returns the tooltip to be displayed for cell gestured by the event.
	 * If vertex was not gestured, returns null.
	 */
	@Override
	...
	/**
	 * Gives the dimension from the origo to the farthest vertexes in right and bottom
	 * added width default margins of vertexes. This is different from actual size of 
	 * the graph, this only tells where the vertexes are situated.
	 */
	...
	/**
	 * Gets first cell for location and otherwise that the method of 
	 * the super class does, this method ignores expanded group cells. 
	 * If user clicked vertex which is in a expanded groups bounds 
	 * the super class method returned cell of the expanded group, 
	 * which is in fact useless, because user wants to select the 
	 * dataset cell, not the group cell.
	 */
	@Override
	...
	/**
	 * Sets UI for graph. The BasicGraphUI does autoscrolling in a really 
	 * annoying way and it can't be disabled very easily. That's why we 
	 * use the own SmartAutoscrollGraphUI
	 
		
	@Override
	public void updateUI() {
		setUI(new SmartAutoscrollGraphUI());
		invalidate();
	}*/
	...
	/** 
	 * Iterates through every vertex and tells them to update their look
	 * to show the selected and non selected items
	 */
	public void dataChanged(DataChangeEvent event) {
		...
		if (event instanceof ContentChangedEvent) {
			...
			this.addOffscreenDirty(this.getBounds());    		
	...
	private void moveCloseToAnnotated(DataBean data) {
		...
		vertex.setPosition(new Point(
				(int)bounds.getX() + GraphVertex.DEFAULT_WIDTH + GraphVertex.DEFAULT_MARGIN_X, 
				(int)bounds.getY()));
	}
	...
	/**
	 * A normal edge without label. I didn't find any better way to disable 
	...
	/* JGraph isn't able to handle L&F changes when it has content, at least for now.
	 * Overriding this method we simply prevent any L&F changes and the initial L&F
	 * will be set in the contructor with a direct call to parents method. This isn't
	 * very nice solution, but hopefully it can be removed later with newer JGraph
	 * versions.
	 * 
