...
/**
 * A GUI component for viewing the data in a graph presentation, where each
...
public class GraphPanel extends JPanel implements ActionListener, 
	PropertyChangeListener, DataChangeListener, AnimatorScrollable{
	...
	public GraphPanel() {
		...
		this.setPreferredSize(new Dimension(
				VisualConstants.LEFT_PANEL_WIDTH, VisualConstants.GRAPH_PANEL_HEIGHT));
		graphScroller.setPreferredSize(new Dimension(
				VisualConstants.LEFT_PANEL_WIDTH, VisualConstants.GRAPH_PANEL_HEIGHT));        
	...
	public void autoZoom(){
		if(autoZoomChecBbox.isSelected()){
			...
			logger.debug("Graph size: " + dim.width + " , " + dim.height +
					" Scroller size: " + graphScroller.getSize().getWidth() + " , " + graphScroller.getSize().getHeight() +
	...
	/**
     * Will return (and if needed, create) the JGraph component that is the
     * very heart of this GraphPanel. Initializes an empty graph and
     * enforces differend kind of UI constraints on it (not editable,
     * not connectable or disconnectable, not sizeable, etc., by user).
     * Also makes the graph antialiased, which has a creat impact on the
    ...
    /**
     * Creates the surrounding scroller component for the graph.
     * When called again, will simply return the existing scroller.
     * The JGraph component will also be initialized from this method by
     * calling getGraph(), if not previously created.
     * 
     * @return A JScrollPane which contains the graph component with
     *         appropriate margins.
     */
    ...
    /**
     * Repaints the graph after scroll to ensure the visibility of all 
     * vertexes
     */
	 ...
	 * 
	 * @param p Point must be given in the 
	 * pixel coordinates of the graph. If this is done after scale of the graph, make
	 * sure that the scaling has been done before calling this.
	 */
	...
	/**
	 * Sets viewport position but checks that graph isn't moved
	...
	/**
	 * @param scale This is set for the scale of graph if it is between zoom
	 * limits. Returns true if the scale was appropriate.
	 */
	...
	private void zoomInToolUsed(){
		...
		if(this.setGraphScale(graph.getScale()*this.ZOOM_FACTOR)){
			...
			center = new Point((int)(viewPos.getX()+view.getWidth()/2), 
					(int)(viewPos.getY()+view.getHeight()/2));
			...
			/* The new scale won't be applied before this method is ended.
			 * The new scale changes the size of the graph, which affects on the
			 * calculations done in method pointToCenter. We couuld estimate the new
			 * size of the graph, but that doesn't help because we still cant scroll to
			 * the new areas.
			 */
