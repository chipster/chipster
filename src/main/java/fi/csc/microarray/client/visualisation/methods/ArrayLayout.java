...
public class ArrayLayout extends VisualisationFactory {
	...
	public JComponent getVisualisation(DataBean dataBean) throws Exception {
		...
		for (int x = 0; x < dimensions[0]; x++) {
			for (int y = 0; y < dimensions[1]; y++) {
				xs[i] = x;
				ys[i] = y;
				zs[i] = transformedIntensities.getFloat((dimensions[1] - y - 1) * dimensions[0] + x); // invert
																										// Y
																										// for
																										// matching
																										// coordinate
		...
		org.jfree.data.contour.ContourDataset data = new org.jfree.data.contour.DefaultContourDataset("data",
				org.jfree.data.contour.DefaultContourDataset.formObjectArray(xs), org.jfree.data.contour.DefaultContourDataset
						.formObjectArray(ys), org.jfree.data.contour.DefaultContourDataset.formObjectArray(zs));
	...
	public boolean canVisualise(DataBean bean) throws MicroarrayException {
		// we can do this for text CEL files only, i.e., must be tabular and
		// Affymetrix
