...
public class VennDiagram extends Visualisation implements PropertyChangeListener{
	...
	public JComponent getVisualisation(List<DataBean> datas) throws Exception {				
		...
		//Every AREA is mapped to the set of String containing the identifiers of each area.
	...
	public boolean canVisualise(List<DataBean> beans) throws MicroarrayException {
		...
		for(DataBean data: beans){
			...
			//FIXME Image is passed through wrongly
			if(!(isTabular && data.queryFeatures("/column/chip.*").exists())){
				return false;
			}
		}
		return true;
	...
	/**
	 * AnnotateListPanel.setSelectedListContentMultipleDatas can't be seen from the VenndiPlot, so we
	 ...
	 * @param dispatchEvent
	 */
	public void setSelectedListContent(List<String> ids,
			Map<DataBean, Set<Integer>> indexes, VenndiPlot venndiPlot,
			boolean dispatchEvent) {
		
		list.setSelectedListContentMultipleDatas(ids,
				 indexes, this, dispatchEvent);
	}
}
