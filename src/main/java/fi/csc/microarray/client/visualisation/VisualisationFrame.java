...
public abstract class VisualisationFrame implements DataChangeListener {
	...
	/**
	 * Removing visualisation panels immediately is crucial for memory
	 * efficiency. Is not explicitly synchronised, should be called only from
	 * EDT.
	 */
		...
		if (e instanceof DataItemRemovedEvent){
			if(datas != null && datas.contains(((DataItemRemovedEvent)e).getDataItem())){
				application.setVisualisationMethod(new VisualisationMethodChangedEvent(
						this, VisualisationMethod.NONE, null, null, type, this));
			}
		}
	}
