...
public class OrderSuperviser {
	...
	public void updateVisibleIndexes() {
		...
		updateVisibleIndexesSubTree(hcPlot.getRowClusteringInfo().getRootNode(), 
				treeToVisible, openRoot, false);
		
		this.setTreeToVisible(treeToVisible);
	}
	
	/**
	 * Method updateVisibleIndexes calls this to go trough the tree recursively. There should
	 ...
	 * @return
	 */
	private boolean updateVisibleIndexesSubTree(HCTreeNodeInfo node, List<Integer> list, 
			boolean openSubTree, boolean firstLeafOfClosed) {
		
		/* It's not very easy job to calculate the relations between indexes of 
		 * partly closed and open tree. Here is one recursive implementation which
		 * seems to work right with tested trees. 
		 * 
		 * OpenSubTree is used to carry information if we are in closed part of 
		 * tree or not, because one closed branch hides also its sub branches, 
		 * whereas their internal state still shows the situation when the tree 
		 * is open.
		 * 
		 * When inside this kind of closed branch, all the leafs get some visible
		 * index as the first one of that closed branch. However, the first has to be
		 * still increased by one from the previous row, because it's still a new 
		 * row in the visualisation. This situation is recognised with variable 
		 * firstLeafOfClosed.
		 */
		...
		if ( (node.getLeftChild() == null) && (node.getRightChild() == null)) {
			...
			if (node.getLeftChild() != null)
				firstLeafOfClosed = updateVisibleIndexesSubTree(node.getLeftChild(), 
						list, openSubTree, firstLeafOfClosed);
			
			if (node.getRightChild() != null)
				firstLeafOfClosed = updateVisibleIndexesSubTree(node.getRightChild(), 
						list, openSubTree, firstLeafOfClosed);
		}
	...
	public int[] getCountOfVisibleReferences(){
		...
		for(int visible : treeToVisible){
			countOfVisibleReferences.set(visible, 
					countOfVisibleReferences.get(visible) + 1);
