...
public class PositionRecordingRenderer extends StandardXYItemRenderer {
	...
	public void drawItem(Graphics2D g2, XYItemRendererState state, 
		...
		//   work in a derived class. it does not support every feature the original did
		...
		//logger.debug("x1: " + x1 + "y1: " + y1 + "transX1: " + transX1 + "transY1: " + transY1);			
		...
		if (getBaseShapesVisible()) {
			...
			if (shape.intersects(dataArea)) {
				...
				for (DataItem2D dItem : selectedItems){
					//if(allItems.indexOf(dItem) == item){
					if(dItem.getIndex() == item){
						selected = true;
						break;
					}
				}
				...
				//logger.debug("drew item " + itemNames.get(item) + " to " + shape.getBounds().x + "," + shape.getBounds().y + ", with h=" + shape.getBounds().height + ", w=" + shape.getBounds().width);  
			}
			entityArea = shape;
