...
/**
 * A GUI component for showing (and some day maybe even editing!) the microarray
 * data - that is, the channels and their intensities. It basically takes the
 * data from a dataset's microarray and puts it to a JTable. Once gene names are
 * implemented in microarrays, this could be used to annotate them (write some
 * describing names and notes for different genes).
 * 
...
public class Spreadsheet extends Visualisation {
	...
	public class SpreadsheetPopupMenu extends JPopupMenu implements ActionListener {
		...
		private JMenuItem createFromSelectionMenuItem;
		private JMenuItem createExcludingSelectionMenuItem;
		...
		public SpreadsheetPopupMenu(MicroarrayTable table) {
			...
			createFromSelectionMenuItem = new JMenuItem("Create dataset");
			createExcludingSelectionMenuItem = new JMenuItem("Create dataset excluding selection");

			copyMenuItem.addActionListener(this);
			annotateMenuItem.addActionListener(this);
			createFromSelectionMenuItem.addActionListener(this);
			createExcludingSelectionMenuItem.addActionListener(this);

			this.add(copyMenuItem);
			this.add(new JSeparator());
			this.add(createFromSelectionMenuItem);
			this.add(annotateMenuItem);
			this.add(createExcludingSelectionMenuItem);
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == annotateMenuItem) {
				...
				table.copy();
			} else if (e.getSource() == createFromSelectionMenuItem) {
				VisualisationUtilities.filterBySelection(getFrame().getDatas());
			} else if (e.getSource() == createExcludingSelectionMenuItem) {
				DataBean data = getFrame().getDatas().get(0);
				if (data != null) {
					VisualisationUtilities.createDatasetExcludingSelection(data);
				}
			}
	 ...
	 * 
	 * @param datas
	 *            The dataset for which to create the table.
	 * @throws Exception
	...
	public JComponent getVisualisation(DataBean data) throws Exception {
		...
		Object[][] rowData = new Object[RestrictModifier.RESTRICT_TO_ROWS < rowCount ? RestrictModifier.RESTRICT_TO_ROWS : rowCount][columns
				.getColumnCount()];
		int row = 0;
	...
	public boolean canVisualise(DataBean bean) throws MicroarrayException {
		...
		if (bean.isContentTypeCompatitible("text/tab", "text/csv")) {
			...
			return !bean.queryFeatures("/embedded-binary-content/").exists(); // might
																				// have
																				// embedded
																				// binary
																				// content
