...
public class CircularView extends View{
			...
			for(int i = 0; i < text.text.length(); i++){
				...
				if(p != null){
			        ...
			        TextLayout tl = new TextLayout(
			        		"" + text.text.charAt(i), g.getFont(), g.getFontRenderContext());
			        
			        Shape rotatedChar = tl.getOutline(
			        		AffineTransform.getRotateInstance(getAngle(p), 
			        				tl.getBounds().getCenterX(), tl.getBounds().getCenterY()));
	...
	private void drawPerimeterLine(Graphics2D g, RectDrawable rect){
		...
		Point.Float endPoint = toCircle(
				new Point.Float(rect.x + rect.width, rect.y));	
		...
		if(startPoint != null && endPoint != null){
			...
			//System.out.println(start  + ", " +  extent);
	...
	private float getAngle(Point.Float p){
		...
		float a =  (float) (Math.atan(
				(p.y - getCenterY()) / (p.x - getCenterX())) + Math.PI / 2);
