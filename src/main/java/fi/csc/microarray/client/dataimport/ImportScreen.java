...
/**
 * The Import tool main class. Includes the GUI and also functions which gives
 * access to the classes which do the file reading/writing etc.
 * 
 * @author mkoski klemela
 * 
 */
public class ImportScreen extends ScreenBase implements ImportScreenModel,
	...
	private Iterator<ImportItem> files;
	...
	class WriteToFileProcess extends RunnableImportProcess {
		...
		public void taskToDo() {
			...
			ImportItem currentFile = null;
			try {
				do {

					// Write file to disk
					conversionModel.writeToFile(informator); // Throws
																// exceptions
					...
					if (application != null) {
						...
						for (File outputFile : conversionModel.getOutputFiles()) {
							ImportItem item = new ImportItem(outputFile, outputFile.getName(), Session.getSession().getDataManager().getContentType("text/tab"));
							dataItems.add(item);
						}
					}
					...
					if (hasNext) {
						...
						checkAndSetInputToConversionModel(currentFile);
					}

					if (hasNext) {
						informator.setMessage("Writing data to disk from file "	+ currentFile.getInputFilename());
					}
			...
			// Catch exceptions
				...
				if (!hasNext) {
					// Reset the import screen and do not show it after
					// resetting
	...
	private void checkAndSetInputToConversionModel(ImportItem currentFile) {
		if (currentFile.getInput() instanceof File) {
			conversionModel.setInputFile((File)currentFile.getInput());						
		} else{
			throw new RuntimeException("import conversion not supported for: " + currentFile.getInput());
		}
	}
	...
	class UpdateTableProcess extends RunnableImportProcess {
		...
		/**
		 * Parse headers and footers? On the first step this should be true and
		 * on the second step false
		 */
		...
		public UpdateTableProcess(ProgressInformator informator,
				boolean hideHeaderFooter) {
			super(informator);
		...
		public void taskToDo() {
			...
			try {
				choppedDataMatrix = conversionModel.chopData(hideHeaderFooter,
						this.getInformator());
			} catch (IOException e) {
			...
			tableFrame.getTable().setData(
					UpdateTableProcess.this.choppedDataMatrix,
					UpdateTableProcess.this.columnTitles);
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					...
					ImportScreen.this.getTableFrame()
	 ...
	 * necessery components. It also sets look'n'feel settings and gets ready to
	 * show the first import step
	 * 
	 */
	public ImportScreen() {
	 ...
	 * 
	 * 
	 * private void updateUI() { toolsFrame.updateUI(); tableFrame.updateUI(); }
	 */
	...
	/**
	 * Goes to the first import step. Calls methods which initialized the table
	 * and other components for the first step.
	 */
	...
	/**
	 * Goes to the second import step. Calls methods which initialized the table
	 * and other components for the second step.
	 */
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == helpButton) {
			if (this.getCurrentStep() == Step.FIRST) {
				application.viewHelp(HelpMapping.MANUAL_ROOT
						+ "import-help.html#step1");
			} else {
				application.viewHelp(HelpMapping.MANUAL_ROOT
						+ "import-help.html#step2");
			}
		}
	...
	public void setImportSession(ImportSession importSession) {
		...
		checkAndSetInputToConversionModel(files.next());
	}
	...
	public void updateTable(boolean hideHeaderFooter) {
		ProgressInformator infoDialog = new InformationDialog("Updating table",
				"", this);
		UpdateTableProcess process = new UpdateTableProcess(infoDialog,
				hideHeaderFooter);
		process.runProcess();
	}
	...
	/**
	 * Reset import screen. This can be used while creating totally new instance
	 * of import screen or just resetting the default.
	 * 
	 * @param setVisibleAfterReset
	 *            set import screen frame visible after it has been reseted
	 */
	public void resetImportScreen(boolean setVisibleAfterReset) {
		...
		// Create new instances of conversion model, column type manager and
		// trimmers
	...
	private void finishButtonPressed() {
		...
		if (columnTypeManager.getChipCount() == 0) {
			JOptionPane.showMessageDialog(this.getFrame(),
					"No chips to import. Select at least one sample.",
					"No chips selected", JOptionPane.ERROR_MESSAGE);
		}

		// Write data to file
		else {
			RunnableImportProcess writeToFileProcess = new WriteToFileProcess(
					new InformationDialog("Writing data to disk",
							"Writing data to disk from file "
									+ conversionModel.getInputFileName(), this));
