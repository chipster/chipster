...
public class OperationDefinition implements ExecutionItem {
	...
	/**
	 * Used to create an operation of origin for imported raw data. Not an actual
	 * operation (in the sense that it would be possible to execute it) but rather a
	 * dummy substitute, without any parameters.
	 */
	...
	/**
	 * An enumeration containing all possible results when evaluating an operation's
	 * suitability to a dataset.
	 */
	...
	static {
		...
		IMPORT_DEFINITION = new OperationDefinition(IMPORT_DEFINITION_ID, "Import data", ToolCategory.IMPORT_CATEGORY,
				"Import data.", false, null);
		CREATE_DEFINITION = new OperationDefinition(CREATE_DEFINITION_ID, "Create a dataset",
				ToolCategory.CREATE_CATEGORY, "Create a new dataset.", false, null);
	}
	...
	public static class InputDefinition {
		 ...
		 * @param isOptional
		 */
		public InputDefinition(String prefix, String postfix, String displayName, String description,
				SADLSyntax.InputType type, boolean isOptional) {
			this.id = prefix;
	 ...
	 * 
	 * @param id          The name of this operation. Should be something that
	 *                    extends the corresponding category name to be more
	 *                    specific (for example, in the category "Normalization",
	 *                    "Lowess" might be a good name of an operation definition,
	 *                    resulting in "Lowess Normalization" when an actual
	 *                    operation is created).
	 * @param description A written description of this operation's purpose.
	 */
	public OperationDefinition(String id, String displayName, ToolCategory category, String description,
			boolean hasSourceCode, String helpURL) {
		this.id = id;
		this.displayName = displayName;
	...
	/**
	 * Like the constructor above, but makes it possible to define operation
	 * definition as local.
	 * 
	 * @param id
	 * @param displayName
	 ...
	 */
	public OperationDefinition(String id, String displayName, ToolCategory category, String description,
			boolean hasSourceCode, String helpURL, boolean isLocal) {
	...
	/**
	 * @return A String representation (actually, just the name) of this operation
	 *         definition, used for showing this on the list.
	 */
	 ...
	 * @param
	 * @return One of the OperationDefinition.Suitability enumeration, depending on
	 *         how suitable the operation is judged.
	 */
	@Override
	...
	public Suitability evaluateBindingSuitability(Iterable<DataBean> datas, List<DataBinding> bindings) {
		...
		/*
		 * If there are several multi inputs of same type, at least one of them must
		 * have a bound databean. We can't require all of them to have, because the
		...
		/*
		 * Special case: Don't care about selected datasets if the tools doesn't take
		 * any inputs, because in this case it's not obvious that users should clear the
		 * selection.
		 */
		if (!inputs.isEmpty()) {
	...
	private boolean bindingsContainsInput(List<DataBinding> bindings, InputDefinition input) {
	...
	/**
	 * Check suitability of a given parameter list. The parameter list can also come
	 * from the Operation object that encapsulates this definition.
	 * 
	 * @param params
	 * @return
	 */
	public static Suitability evaluateParameterSuitability(List<Parameter> params) {
		for (Parameter param : params) {
			...
			if (!param.isOptional() && (param.getValue() == null || param.getValue().equals(""))) {
				return Suitability.EMPTY_REQUIRED_PARAMETERS;
			}
		}
	 ...
	 * concrete inputs (as chosen by user) using greedy and order-based algorithm.
	 * Formal inputs are processed in order and first fitting concrete input is
	 * bound to them. If formal input can have multiple concrete inputs, then all
	 * fitting ones are bound. Always at least one concrete input must be bound, a
	 * single concrete input cannot be bound multiple times and in the end all
	 * concrete inputs must be bound.
	 * 
	 * @param inputValues no changes are made to this parameter
	...
	public BindingResult bindInputs(Iterable<DataBean> inputValues) {
		...
		if (!unboundMetadataDefinitions.isEmpty()) {
			...
			LinkedList<DataBinding> phenodataBindings = new LinkedList<DataBinding>(); // need this to prevent
			...
			while (bindingIterator.hasNext() && unboundMetadataIterator.hasNext()) {
				...
				if (metadata != null) {
					phenodataBindings
							.add(new DataBinding(metadata, unboundMetadata.getID(), ChipsterInputTypes.PHENODATA));
	...
	public static boolean doBackwardsCompatibleTypeCheck(InputType type, DataBean data) {
		...
		if (type.equals(ChipsterInputTypes.AFFY)) {
			...
			return data.hasTypeTag(MicroarrayModule.TypeTags.FASTA_FILE)
					|| data.hasTypeTag(MicroarrayModule.TypeTags.FASTA_GZ_FILE);

		} else if (type.equals(ChipsterInputTypes.GTF)) {
			return data.hasTypeTag(MicroarrayModule.TypeTags.GTF_FILE)
					|| data.hasTypeTag(MicroarrayModule.TypeTags.GTF_GZ_FILE);
			...
			return data.hasTypeTag(MicroarrayModule.TypeTags.MOTHUR_GROUPS)
					|| data.hasTypeTag(MicroarrayModule.TypeTags.MOTHUR_GROUPS_GZ);
			...
			return data.hasTypeTag(MicroarrayModule.TypeTags.MOTHUR_COUNT)
					|| data.hasTypeTag(MicroarrayModule.TypeTags.MOTHUR_COUNT_GZ);
	...
	/**
	 * @return true if this operation should be run in LocalTaskExecutor instead of
	 *         server
	 */
	...
	/**
	 * Checks if the operation defined by this operation definition is safe to run
	 * as batch. Batch run is tricky for input binding and input sensitive parameter
	 * evaluation, so we do strict checks and only allow "simple" operations to be
	 * run as batch. This is done to not confuse the user and create surprising
	 * effects to workflows, for example.
	 * 
