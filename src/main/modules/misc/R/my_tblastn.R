# TOOL my_tblastn.R: "Translated protein BLAST against users of nucleotide sequence set" (Heuristic tool to search hits from users own nucleotide sequence set using protein query. Your query sequence set can contain 100000 nucleotide sequences in maximum.)
# INPUT query.fa: "Query sequences" TYPE GENERIC
# INPUT dbnuc.fa: "Database sequences" TYPE GENERIC
...
general.parameters <- paste("-chipster_path /opt/chipster -no_slurm -query query.fa -out blast_results -dbnuc dbnuc.fa -db_gencode", db_gencode, "-evalue ", evalue, "-matrix", matrix, "-word_size" , word_size , "-seg" , seg , "-outfmt" , outfmt, "-max_intron_length",  max_intron_length)
