# TOOL ncbi_blastx.R: "Translated nucleotide BLAST" (Heuristic tool to search hits from the NCBI protein sequence databases using a translated nucleotide query. Your query sequence set can contain 10 nucleotide sequences in maximum.)
...
# PARAMETER db: "Database" TYPE [nr: "NCBI non-redundant proteins", pdb: "PDB proteins", swissprot: "UniProt / Swiss", refseq_protein: "RefSeq reference proteins", pat: "Patented protein sequences", env_nr: "Metagenomic proteins", tsa_nr: "Transkriptome Shotgun Assembly proteins"] DEFAULT swissprot (Database to search.)
...
# PARAMETER OPTIONAL query_gencode: "Genetic code" TYPE [1: "Standard", 2: "Vertebrate Mitochondrial", 3: "Yeast Mitochondrial", 4: "Mold Mitochondrial", 5: "Invertebrate Mitochondrial", 6: "Ciliate Nuclear", 9: "Echinoderm Mitochondrial", 10: "Euploid Nuclear", 11: "Bacteria and Archaea", 12: "Alternative Yeast Nuclear", 13: "Ascidian Mitochodrial", 14: "Flatworm Mitochondrial", 15: "Blepharisma Macronuclear"] DEFAULT 1 (Genetic code to use to translate query.)
...
command.start <- paste(pb.binary, "blastx")
...
general.parameters <- paste("-chipster_path /opt/chipster -remote -no_slurm -query query.fa -out blast_results -db ", db, "-query_gencode", query_gencode, "-evalue ", evalue, "-matrix", matrix, "-word_size" , word_size , "-seg" , seg , "-outfmt" , outfmt, "-max_intron_length",  max_intron_length)
