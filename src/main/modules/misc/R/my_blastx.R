# TOOL my_blastx.R: "Translated nucleotide BLAST against users own protein sequcense set." (Heuristic tool to search hits from the a protein sequence set using a translated nucleotide query. Your query sequence set can contain 100000 nucleotide sequences in maximum.)
...
general.parameters <- paste("-chipster_path /opt/chipster -no_slurm -query query.fa -out blast_results -dbprot  dbprot.fa -query_gencode", query_gencode, "-evalue ", evalue, "-matrix", matrix, "-word_size" , word_size , "-seg" , seg , "-outfmt" , outfmt, "-max_intron_length",  max_intron_length)
