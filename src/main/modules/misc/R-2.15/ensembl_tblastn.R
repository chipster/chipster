# TOOL ensembl_tblastn.R: "Translated protein BLAST against genome or transcriptome of a given species in Ensembl" (Heuristic tool to search hits from users own nucleotide sequence set using protein query. Your query sequence set can contain 100000 nucleotide sequences in maximum.)
...
# PARAMETER species: "Species" TYPE STRING DEFAULT "None" (Give the name of the species that is available in Enesembl or Ensembl genomes database. For example homo_sapiens or mus_musculus.) 
# PARAMETER ome: "Target sequence set" TYPE [ -ensembl_dna: "full genomic sequence", -ensembl_cdna: "just coding sequeces" ] DEFAULT -ensembl_dna ( Define, if the full genome or just the coding sequences are used as the target sequence set.) 
...
# PARAMETER OPTIONAL db_gencode: "Genetic code" TYPE [1: "Standard", 2: "Vertebrate Mitochondrial", 3: "Yeast Mitochondrial", 4: "Mold Mitochondrial", 5: "Invertebrate Mitochondrial", 6: "Ciliate Nuclear", 9: "Echinoderm Mitochondrial", 10: "Euploid Nuclear", 11: "Bacteria and Archaea", 12: "Alternative Yeast Nuclear", 13: "Ascidian Mitochodrial", 14: "Flatworm Mitochondrial", 15: "Blepharisma Macronuclear"] DEFAULT 1 (Genetic code to use to translate query.)
...
# PARAMETER OPTIONAL max_intron_length: "Length of the largest intron allowed" TYPE INTEGER DEFAULT 0 ( Length of the largest intron allowed in a translated nucleotide sequence when linking multiple distinct alignments. A negative value disables linking)
...
command.start <- paste(pb.binary, "tblastn")
...
general.parameters <- paste("-chipster_path /opt/chipster -no_slurm -query query.fa -out blast_results")
general.parameters <- paste(general.parameters, ome, species )
general.parameters <- paste(general.parameters, "-evalue ", evalue)
general.parameters <- paste(general.parameters, "-seg" , seg)
general.parameters <- paste(general.parameters, "-outfmt" , outfmt)
general.parameters <- paste(general.parameters, "-db_gencode", db_gencode, "-matrix", matrix)
general.parameters <- paste(general.parameters, "-word_size" , word_size , "-max_intron_length",  max_intron_length)
