...
# PARAMETER std: "Standard deviation" TYPE DECIMAL FROM 1 DEFAULT 75 (The standard deviation of the position distribution centered at the position specified by the position tag.)
# PARAMETER wf: "Weighting factor" TYPE DECIMAL FROM 0 TO 1 DEFAULT 0.2 (The value for weighting the data, a value between 0 and 1. Recommended values: 0.2 for ChIP-seq/ChIP-exo, 0.01 for PBM data.)
# PARAMETER starts: Starts TYPE INTEGER FROM 1 TO 100 DEFAULT 20 (The number of pre-optimization runs. Default value is fine for most applications.)
# PARAMETER motifwidth: "Initial motif width" TYPE INTEGER FROM 3 TO 30 DEFAULT 15 (The motif width that is used initially, may be adjusted during optimization.)
# PARAMETER fgOrder: "Markov order of motif model" TYPE INTEGER FROM 0 TO 3 DEFAULT 0 (The Markov order of the model for the motif. A value of 0 (default\) specifies a position weight matrix (PWM\) or position-specific scoring matrix (PSSM\), a value of 1 specifies a weight array matrix (WAM\) model.)
# PARAMETER bgOrder: "Markov order of background model" TYPE INTEGER FROM -1 TO 5 DEFAULT -1 (The Markov order of the model for the background sequence and the background sequence, -1 defines uniform distribution.)
