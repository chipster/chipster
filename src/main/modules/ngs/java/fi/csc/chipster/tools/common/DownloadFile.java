...
public class DownloadFile extends JavaCompJobBase {
	...
	/*
	 * We definitely don't wan't to enable 'file' protocol. Other protocols might be
	 * fine as long as the isLocalhost() check below is sufficient for it.
	 */
	...
	public String getSADL() {
		return "TOOL fi.csc.chipster.tools.common.DownloadFile.java: \"Download file from URL directly to server\" (Download a file from a URL address to the Chipster server. The URL must be visible to Chipster server. If it's not, use client's 'Import from URL' functionality instead.)"
				...
				+ "PARAMETER paramUrl: \"URL\" TYPE UNCHECKED_STRING (URL to download)\n"
				+ "PARAMETER OPTIONAL paramFileExtension: \"Add a file extension\" TYPE [" + CURRENT
				+ ": \"Keep current\", bam: \"BAM\", fa: \"FASTA\", fastq: \"FASTQ\", gtf: \"GTF\"] DEFAULT " + CURRENT
				+ " (The output file is named according to the last part of the URL. If it doesn't contain a correct file extension, select it here so that the file type is recognized correctly.)\n"
				+ "PARAMETER OPTIONAL paramCheckCerts: \"Require valid SSL certificate\" TYPE [" + YES + ": \"Yes\", "
				+ NO + ": \"No\"] DEFAULT " + YES
				+ " (Disable if the https server has a self-signed ssl certificate which you trust.)\n";
	}
	...
	protected void execute() {
		...
		try {
			...
			if (checkCerts) {
				// needed for old chipster which configures socket factory
				// doesn't do anything in the new chipster
				// can be removed when old not in use anymore
