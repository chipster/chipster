...
colnames(dat)[-(1:4)] <- sprintf('chip.microarray%.3i', 1:length(filenames))
...
dat2 <- data.frame(cgh.nor@featureData@data)
colnames(dat2) <- c('chromosome', 'start', 'end')
dat2 <- cbind(dat2, copynumber(cgh.nor))
...
phenodata <- data.frame(sample=filenames, chiptype='not applicable', experiment='cna_seq', groups='', stringsAsFactors=FALSE) # description?
