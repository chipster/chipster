...
# INPUT file.a: "BED/BAM file" TYPE GENERIC
# INPUT file.b: "Genome file" TYPE GENERIC
# OUTPUT genomecoveragebed.txt 
# PARAMETER ibam: "File A is BAM format" TYPE [yes, no] DEFAULT no (The input file is in BAM format. Note: BAM must be sorted by position.)
...
# PARAMETER OPTIONAL max: "Combine positions by depth" TYPE TYPE [yes, no] DEFAULT no (Combine all positions with a depth >= max depth into a single bin in the histogram. Option is relevant only for histogram output.)
...
if (abam == "no") {options <- paste(options, "-i file.a -b file.b")}
