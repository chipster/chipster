...
# PARAMETER band.with: "Bandwidth" TYPE INTEGER FROM 1 TO 1000 DEFAULT 200 (The scanning window size, typically half the average fragment size of the DNA)
...
# PARAMETER build.model: "Peak model" TYPE [yes, no] DEFAULT yes (If enabled, a peak model is built from the data. Disabling model building means the shiftsize has to be guessed. In Chipster the shift size is set to half the bandwidth.)
...
# with two experiment condition, like treatment and #
# control.                                          #
#                                                   #
#####################################################
...
parseMACSResultsPOS <- function(name, final=FALSE){
	if( final ) {
		...
		## Choose columns
#		output <- output[c("chr","start","end","length","fold_enrichment", "X.10.log10.pvalue.","tags","summit","FDR(%)")]
		## Fix colnames
...
parseMACSResultsNEG <- function(name, final=FALSE){
	if( final ) {
		...
		## Choose columns
#		output <- output[c("chr","start","end","length","fold_enrichment", "X.10.log10.pvalue.","tags","summit")]
		## Fix colnames
		colnames(output)[7] <- "neg10xlog10pvalue"
		## Sort the result according to the -10xlog10(pvalue)
		output <- output[ order(output[,5], decreasing=TRUE), ]
