...
# PARAMETER replicates: "Disregard replicates" TYPE [yes, no] DEFAULT no (It is necessary to have biological replicates of each experiment condition in order to estimate the biological and experimental variability of the data. If biological replicates are available for only one of the conditions, DESeq will estimate variability using the replicates of that single condition. However, this is only an approximation and the reliability of results may suffer as a consequence. In the case where there are no replicates at all, the variance is estimated using the single samples from the different conditions as replicates. This approximation is even less reliable and affects results accordingly.)
